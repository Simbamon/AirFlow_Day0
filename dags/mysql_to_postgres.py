...
@task()
def get_src_tables():
    hook = MsSqlHook(mssql_conn_id="localhost_sql_server")
...
def load_src_data(tbl_dict: dict):
    conn = BaseHook.get_connection('localhost_postgres')
    engine = create_engine(f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
    all_tbl_name = []
    start_time = time.time()
    #acess the table_name element in dictionaries
...
with DAG(
    ...
    with TaskGroup("transform_src_product", tooltip="Transform and stage data") as transform_src_product:
        ...
        #define task order - These tasks can run parallel
